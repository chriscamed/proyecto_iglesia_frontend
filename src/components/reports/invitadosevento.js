...
import InvitadoTotal from './InvitadoTotal';

class ModalInvitado extends Component {
  ...
  cargarDatosTotal = () => {
    const config = {
        ...
        Axios.get('http://localhost:5000/reportes/asistenciainvitados/?fechaInicio='+this.state.fechaInicial+'&'+'fechaFin='+this.state.fechaFinal, config)
        .then(response => {
  ...
  cambiardatos = () =>{
    if(this.state.data.length <= 0){
        ...
        return(
            ...
            <InvitadoTotal data={this.state.data}/>
